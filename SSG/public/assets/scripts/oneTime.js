(function() {
  ...
  const bindRoutesBtn = () => {
    ...
    if (tab0) {
      tab0.addEventListener("click", getOneTimeRoutes);
    }
  ...
  const getOneTimeRoutes = () => {
    apiCaller("/api/one-time-routes").then(resp => {
      let tableElem = document.querySelector("#one-time-routes");
      const markup = `
  ...
  const submitUpdatedRoutes = e => {
    ...
    if (!textAreaElem.value) {
      ...
      apiCaller("/api/update-one-time-routes", {
  ...
  const addRoutes = e => {
    ...
    if (!textAreaElem.value) {
      ...
      apiCaller("/api/add-one-time-routes", {
        routes: validatedRoutes
      }).then(() => {
  ...
  window.deleteRoute = route => {
    apiCaller("/api/delete-one-time-route", {
      route
    }).then(() => {
  ...
  window.filterTable = () => {
    ...
    if (searchBox) {
      ...
      const tableBody = document.querySelectorAll("#one-time-routes tbody tr");
